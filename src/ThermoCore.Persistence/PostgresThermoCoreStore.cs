...
/// <summary>PostgreSQL-backed ThermoCore store (DATA-005).</summary>
public sealed class PostgresThermoCoreStore : IThermoCoreStore, IDisposable
{
    private readonly string _connectionString;
    private bool _disposed;

    public PostgresThermoCoreStore(string connectionString)
    {
    ...
    public void EnsureCreated()
    {
        ...
        command.CommandText =
            ...
            CREATE TABLE IF NOT EXISTS simulation_summaries (
                ...
                aggregated_energy_residual_j DOUBLE PRECISION NOT NULL,
                aggregated_water_residual_kg DOUBLE PRECISION NOT NULL,
                water_balance_passed BOOLEAN NOT NULL,
                energy_balance_passed BOOLEAN NOT NULL,
                final_water_tank_content_kg DOUBLE PRECISION NULL,
                created_at_utc TEXT NOT NULL,
            ...
            CREATE TABLE IF NOT EXISTS calibration_runs (
                ...
                initial_objective DOUBLE PRECISION NOT NULL,
                final_objective DOUBLE PRECISION NOT NULL,
            ...
            CREATE TABLE IF NOT EXISTS result_series_payloads (
                simulation_summary_id TEXT NOT NULL PRIMARY KEY,
        ...
        Guid configurationVersionId)
    {
        ...
        command.Parameters.AddWithValue("succeeded", summary.Succeeded);
        ...
        command.Parameters.AddWithValue("waterPass", summary.WaterBalancePassed);
        command.Parameters.AddWithValue("energyPass", summary.EnergyBalancePassed);
        ...
        AwgSimulationRunResult run)
    {
        ...
        var storageLocation = $"pg://result_series_payloads/{simulationSummaryId:N}";
        var values = collected.Channels.ToDictionary(
            c => c.Definition.Id,
            c => c.Values,
            StringComparer.Ordinal);
        var payload = ResultSeriesPayloadCodec.Encode(values);

        using var connection = Open();
        using var tx = connection.BeginTransaction();

        using (var deleteChannels = connection.CreateCommand())
        {
            deleteChannels.Transaction = tx;
            deleteChannels.CommandText = "DELETE FROM result_series WHERE simulation_summary_id = @id;";
            deleteChannels.Parameters.AddWithValue("id", simulationSummaryId.ToString("N"));
            deleteChannels.ExecuteNonQuery();
        }

        using (var upsertPayload = connection.CreateCommand())
        {
            upsertPayload.Transaction = tx;
            upsertPayload.CommandText =
                """
                INSERT INTO result_series_payloads (simulation_summary_id, payload)
                VALUES (@id, @payload)
                ON CONFLICT (simulation_summary_id) DO UPDATE SET payload = EXCLUDED.payload;
                """;
            upsertPayload.Parameters.AddWithValue("id", simulationSummaryId.ToString("N"));
            var blob = upsertPayload.Parameters.Add("payload", NpgsqlDbType.Bytea);
            blob.Value = payload;
            upsertPayload.ExecuteNonQuery();
        }
        ...
        foreach (var channel in collected.Channels)
        {
            ...
            insert.Parameters.AddWithValue("location", storageLocation);
    ...
    public StoredResultSeriesBundle? GetResultSeries(Guid simulationSummaryId, bool loadValues = true)
    {
        ...
        if (loadValues)
        {
            ...
            payloadCommand.CommandText =
                """
                SELECT payload FROM result_series_payloads WHERE simulation_summary_id = @id;
                """;
            payloadCommand.Parameters.AddWithValue("id", simulationSummaryId.ToString("N"));
            var blob = payloadCommand.ExecuteScalar() as byte[]
                ?? throw new InvalidOperationException(
                    $"Missing result series payload for '{simulationSummaryId:N}'.");
            values = ResultSeriesPayloadCodec.Decode(blob);
        }
    ...
    private static StoredSimulationSummary ReadSummary(NpgsqlDataReader reader)
    {
        return new StoredSimulationSummary
        {
            ...
            WaterBalancePassed = reader.GetBoolean(8),
            EnergyBalancePassed = reader.GetBoolean(9),
